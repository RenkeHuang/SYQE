...
\subsection{State Preparation} Given a qubit Hamiltonian after the Jordan-Wigner\cite{jordan1993paulische} or the Bravyi-Kitaev encoding,\cite{Seeley:2012em}  
\begin{equation}
\hat{H}=E_{0}+\sum_{l} h_{l} \bigotimes_{k=1}^{n} \sigma_{k}^{\{I, X, Y, Z\}}
\end{equation}
...
\begin{equation}
e^{-i t \hat{H}}=\left(\prod_{l} e^{-i \frac{t h_{l}}{m} \bigotimes_{k} \sigma_{k}}\right)^{m} 
...
We argue that the \methodabbr method is likely to achieve better error scaling than the Trotter error, because it has the variational optimization of the state when solving the generalized eigenvalue problem in a subspace, while variational methods appear to have error resilience to some extent.\cite{McClean:2015bs, OMalley:2016dc}
...
\bibliography{bibs/jan26export.bib,bibs/extra.bib,bibs/qkdrefs.bib}{}
